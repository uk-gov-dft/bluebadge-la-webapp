...
public abstract class AssertionHelper {
  ...
  /**
   * Repeatedly attempts to assert given actual and expected values until the assertion is met or a
   * timeout is reached, delegating to {@linkplain org.junit.Assert#assertThat(String, Object,
   * Matcher)}.
   *
   * <p>When the assertion is resolved before the timeout the method simply exits; otherwise it
   * re-throws the exception caught during most recent assertion attempt.
   */
  public static <T> void assertWithinTimeoutThat(
      final String assertionFailureMessage,
    ...
    if (throwable != null) {
      final String message =
          format(
              "Assertion failed to evaluate within timeout of {1} seconds. Assertion: {0}",
