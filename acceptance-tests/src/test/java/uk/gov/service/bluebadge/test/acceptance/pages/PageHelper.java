...
public class PageHelper {
  ...
  private List<WebElement> findElements(
      final Function<By, List<WebElement>> findElements, final By bySelector) {
    try {
  ...
  public void waitUntilVisible(WebElement webElement) {
    try {
      ...
      throw new RuntimeException(
          "Timeout while waiting for element to become visible: '" + webElement + "'", exception);
    }
  }

  /** This takes care of element being removed/refreshed from DOM */
  public <T> T waitForElementUntil(ExpectedCondition<T> condition) {
    return pollWithTimeout().until(ExpectedConditions.refreshed(condition));
  }
